...
dojo.declare("atg.widget.previewPicker", [dijit.Dialog], {
  ...
  templatePath: "/WebUI/dijit/previewLauncher/templates/previewPicker.jsp",
  ...
  layout: function(){
    if (this.isFF() == true){
      var underlayWrapper = document.getElementsByClassName("dijitDialogUnderlayWrapper");
      if (underlayWrapper) 
        underlayWrapper[0].style.top = 0;
      var underlay = document.getElementsByClassName("dijitDialogUnderlay");
      if (underlay){
        var viewport = dijit.getViewport();
        underlay[0].style.height = viewport.h + "px";
      }
    }else{
  ...
  _position: function(){
    if (this.isFF() == true){
      ...
      style.left = Math.floor((viewport.w - mb.w)/2) + "px";
      style.top = Math.floor((viewport.h - mb.h)/2) + "px";
      this.layout();
    }else{
      this.inherited(arguments);
    }
  ...
  setupList: function(obj, type){ 
    ...
    if (this.userListElement != null && this.urlListElement != null && (!this.sitesEnabled || this.siteListElement != null) ) {
      ...
      var labelsWidth = Math.max(dojo.style(this.userChosenLabel, "width"), dojo.style(this.urlChosenLabel, "width"));
      if (this.sitesEnabled == true) {
        this.ppPicker.appendChild(this.siteListElement);
        labelsWidth = Math.max(labelsWidth, dojo.style(this.siteChosenLabel, "width"));
      }
      else {
        ...
        // ASSETMGR-170011 fix. 
        // dojo.byId, dojo.query, getElementById sometimes returns a "wrong" node "siteChosen" under IE
        // Manipulation with this node (exaMp. innerHTML="") don't affect UI. Node is stayed unchanged.
        // Only in case of this fix node can be changed
        var siteNode = this.getElementById("siteChosen", this.domNode);
	    if (siteNode)
          siteNode.innerHTML="";
      }
      this.ppPicker.appendChild(this.urlListElement);
      dojo.style(this.userChosenLabel, "width", new String(labelsWidth) + "px");
      dojo.style(this.urlChosenLabel, "width", new String(labelsWidth) + "px");
      dojo.style(this.siteChosenLabel, "width", new String(labelsWidth) + "px");
    }
  },
  ...
  serverLaunchCheck: function(){
    ...
    var url = this.selectedUrl + "&atgPreviewId=" + this.selectedUser;
    var escapedUrl = escape(url);
    escapedUrl = escapedUrl + (this.sitesEnabled == true ? "&stickySite=setSite&pushSite=" + this.selectedSite : "");
    _this = this;
    dojo.xhrPost ({
        url: '/WebUI/preview/setPreviewParams.jsp',
        content: {"atgPreviewId": this.selectedUser, "url": escapedUrl, "site": this.selectedSite, "assetURI": this.assetURI },
        handleAs: 'json',
        preventCache: true,
  ...
  getElementById: function(id, node){
   var result = null;
