...
dojo.declare("atg.widget.assistance.Base", [dijit._Widget], {
    ...
    //console.debug("EA | Popup being created");
    var helpTrigger = document.createElement("A");
    var helpImg = document.createElement("IMG");
    helpImg.src = this.popupIcon;
    helpTrigger.id = "trigger_"+obj.id;
    helpTrigger.setAttribute("href", "#");
    helpImg.className = "ATG_EA_helpTrigger";
    helpTrigger.appendChild(helpImg);
    ...
    this.connect(this.helpTrigger, "onfocus", "_handlePopupTrigger");

    this.connect(this.helpTrigger, "onblur", "_mouseOut");

    this.connect(this.helpTrigger, "onclick", "_returnFalse");
	},
	
	_handlePopupTrigger: function(evt) {

    var helpObject = evt.target.helpObj || evt.target.parentNode.helpObj;
