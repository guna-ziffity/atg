<%--
  An expression editor for LiteralExpression instances with tooltip option.
  ...
  @version $Id: //product/WebUI/version/11.2/src/web-apps/WebUI/expreditor/targeting/siteGroupTypePriorityEditor.jsp#1 $$Change: 946917 $
...
<dspel:page>
  ...
  <c:set var="literalValue" value="${fn:replace(paramExpression.value, '\\\'', '\\\\\\'')}"/>
  <c:set var="literalValue" value="${fn:replace(literalValue, '\"', '&amp;quot;')}"/>
  <a id="<c:out value='${labelId}'/>"
     ...
     onclick="javascript:atg.expreditor.displayTerminalMenu({containerId:        '<c:out value="${paramContainer}"/>',
                                                          ...
                                                          literalValue:       '<c:out value="${literalValue}" escapeXml="false"/>',
  ...
  <dspel:include page="/expreditor/terminalMenu.jsp">
