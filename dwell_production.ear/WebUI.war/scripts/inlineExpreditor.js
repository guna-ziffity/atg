...
atg.expreditor.applyPendingChanges = function(callback) {
  ...
  atg.expreditor._menuAnchor.innerHTML = replaceSpecialCharacters(textValue);
  ...
  requestUrl += "&value=" + encodeURIComponent(replaceSpecialCharacters(atg.expreditor._textField.value));
...
atg.expreditor.onLiteralChange = function(evt, target) { // NB: Fix this API
  ...
  if (target) {
    ...
    requestUrl += "&terminalId=" + atg.expreditor._terminalId;
    requestUrl += "&operation=" + operation;
    requestUrl += "&value=" + encodeURIComponent(replaceSpecialCharacters(target.value));
...
atg.expreditor.onLiteralKeyDown = function(evt) {
  ...
  // If the backspace or delete key is pressed on IE, we need to indicate that a change is
...
atg.expreditor.doPerformTargetingOperation = function() {
  ...
  if (args.rulesetGroupType)
    requestUrl += "&type=" + args.rulesetGroupType;
  requestUrl += "&operation=" + args.operation;
  
  callback = editor.callback;
  callbackData = editor.callbackData;

  if (args.callback) {
    callback = function(data, response) { 
      args.callback();
      if (editor.callback) {
        editor.callback(editor.callbackData, response)
      }
    }
  }

  atg.expreditor._issueRequest(requestUrl, containerId, callback, callbackData);
}
...
// These symbols might cause problems in expressioneditor's tokens
function replaceSpecialCharacters(testValue) {
